...
def carregar_dados_2022():
    try:
        ...
        if "geocodigo" in df.columns and "nome" in df.columns:
            ...
            df["UF_code"] = pd.to_numeric(
                df["UF_code"], errors="coerce"
            ).fillna(0)
            df["sg_uf"] = df["UF_code"].map(uf_map).fillna("ND")
            ...
            df["qt_votos_nom_validos"] = (
                pd.to_numeric(df[col_votos], errors="coerce")
                .fillna(0)
                .astype(int)
                if col_votos
                else 0
            ...
            df["nm_candidato"] = (
                "Presidente mais votado (" + df["sg_partido"] + ")"
            )
...
def obter_df_por_ano(ano):
    if ano == "2022 (Presidencial)":
        return carregar_dados_2022()
    else:
...
with tab2:
    ...
    if df_cidade_ano.empty:
        st.warning(
            f"⚠️ Dados não encontrados para a eleição de **{ano_cidade_sel}**."
        )
    else:
        ...
        if cid_sel:
            ...
            st.markdown(
                f"### 📍 **{cid_sel}** — Eleições **{ano_cidade_sel}**"
            )
